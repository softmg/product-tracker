...
4. **If tests fail**: call **Debugger** to fix issues → re-run Test-Runner (max 3 retries)
...
3. **Retry on failure**: If test-runner fails → call debugger → re-run test-runner (max 3 attempts)
4. **Same chat**: All happens in current conversation, visible to user
5. **Pass context forward**: Tell each agent what the previous one did
