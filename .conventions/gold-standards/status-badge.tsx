// Gold Standard: Small display component backed by a typed status-to-UI mapping
// Pay attention to: typed Record config, props interface, cn() merge for optional className
...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  return <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", info.colorClass, className)}>{info.label}</span>
}
